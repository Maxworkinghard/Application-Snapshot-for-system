...
pub fn system_audio_capability() -> super::CapabilityStatus {
    match system_audio_source() {
        Ok(_) => super::CapabilityStatus {
            available: true,
            detail: "PulseAudio/PipeWire：录制默认播放设备的系统混音".into(),
        },
        Err(detail) => super::CapabilityStatus {
            available: false,
            detail,
        },
    }
...
pub fn microphone_capability() -> super::CapabilityStatus {
