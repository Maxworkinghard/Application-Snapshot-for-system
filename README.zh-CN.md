...
- **桌面伴侣**：在「桌面伴侣」页导入一个装着 GIF 的 `.zip`，或者单个 GIF。一个动作一个文件，文件名含 `idle` 的作为默认形象。只支持 GIF，压缩包里的其他格式会被跳过（各平台 WebView 能播的视频编码不一样，所以暂不支持视频）。压缩包上限 100 MB，单个 GIF 上限 50 MB。桌宠在桌面上的大小在「偏好设置 → 桌面宠物」里调（50%–300%）。
