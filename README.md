...
| 提示词润色（剪贴板草稿 → 确认 → 大模型改写 → 写回，支持撤销/停止） | ✅ | ✅ | ✅ |
| 托盘 / 菜单栏入口 | ✅ 菜单栏 | ✅ 托盘 | ✅ StatusNotifierItem 托盘 |
| 轻量 Toast / 桌面通知 | ✅ | ✅ | ✅ |
| 剪贴板 60 秒自动清空 | ✅ | ✅ | ✅ |

润色默认参数三端一致：`max_tokens = 16384`、`temperature = 0.3`、超时 180 秒；提示词改写规则与流程（确认 → 写回 → 撤销）保持同一份行为约定。

## macOS 版功能

- `⌥⇧2`：立即截取当前应用的最前窗口并复制（PNG + TIFF 双格式）
- `⌥⇧R`：录制当前应用窗口为 MP4（H.264，菜单栏或浮动控制条停止）
- 菜单栏 `润色 Prompt`：改写剪贴板中的提示词草稿，确认后写回剪贴板，可撤销
